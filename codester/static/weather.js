...
function closeHours(focus = false) {
  ...
  if (focus) $('#weather-toggle').focus();
}
function render(data) {
  const available = Boolean(data.current);
  $('#weather-current').hidden = !available;
  $('#weather-setup').hidden = available;
  $('#weather-setup').textContent = data.state === 'disabled' ? 'Set up weather' : 'Weather settings';
  ...
  $('#weather-location').textContent = data.location.split(',')[0];
  $('#weather-location').title = data.location;
  $('#weather-toggle').disabled = !data.hours.length;
...
async function refresh() {
  ...
  try { render(await api('/api/weather', {timeout:15000})); lastSuccess = Date.now(); }
    ...
    else $('#weather-status').textContent = 'Out of date';
  } finally { busy = false; }
}
$('#weather-toggle').addEventListener('click', () => {
  const expanded = $('#weather-hours-panel').hidden;
  $('#weather-hours-panel').hidden = !expanded;
  $('#weather-toggle').setAttribute('aria-expanded',String(expanded));
});
